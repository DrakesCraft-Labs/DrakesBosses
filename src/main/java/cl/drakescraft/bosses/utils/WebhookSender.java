...
public final class WebhookSender {
    ...
    public static void sendAsync(JavaPlugin plugin, String url, String jsonBody) {
        ...
        try {
            var req = HttpRequest.newBuilder(URI.create(trimmed))
                    ...
                    .header("User-Agent", "DrakesBosses/1.0.0")
            ...
            httpClient.sendAsync(req, HttpResponse.BodyHandlers.ofString()).whenComplete((resp, err) -> {
                if (err != null) {
                    ...
                    if (code == 429) {
                        ...
                        plugin.getLogger().warning("[DrakesBosses] Discord 429 rate-limit (" + waitMs + " ms). Se descarta este aviso para no bloquear hilos.");
                    } else if (code < 200 || code >= 300) {
                        plugin.getLogger().warning("[DrakesBosses] Discord responded with HTTP " + code + " for webhook delivery.");
                    }
                }
            });
        } catch (Exception e) {
            plugin.getLogger().log(Level.WARNING, "[DrakesBosses] Could not post to Discord webhook", e);
        }
    }
    ...
    public static void sendSyncBestEffort(JavaPlugin plugin, String url, String jsonBody) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.FINE, "[DrakesBosses] Shutdown webhook not delivered: " + e.getMessage());
        }
    }
    ...
    private static void warnTransportOnce(JavaPlugin plugin, Throwable error) {
        ...
        plugin.getLogger().log(Level.WARNING, "[DrakesBosses] Discord webhook HTTP task failed: " + error.getMessage());
    }
}
